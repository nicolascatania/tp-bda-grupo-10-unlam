...
BEGIN
	IF NOT EXISTS (SELECT 1 FROM dominio.actividad WHERE ID_actividad = @id_actividad)
		BEGIN
			...
			RETURN;
		END
	IF UPPER(@dia) NOT IN ('LUNES','MARTES','MIÔøΩRCOLES','JUEVES','VIERNES','SÔøΩBADO','DOMINGO')
		BEGIN
			RAISERROR('El dÔøΩa debe ser un dÔøΩa de la semana vÔøΩlido.', 16, 1);
			RETURN;
		END
...
BEGIN
    ...
    BEGIN
        RAISERROR('La inscripciÔøΩn a actividad especificada no existe.', 16, 1);
        RETURN;
    END
...
/* ANULACION DE FACTURA, por motivos legales, una factura puede ser anulada para anular su validez, debe hacerse x medio de ARCA (pero queda fuera del alcance del tp), sin embargo, modelamos esto 
	ya que el sistema debe ser capaz de identificar las facturas que deben ser llevadas a hacer el tr·mite de anulaciÛn, derivando en nota de credito o en simple reembolso*/
...
--=====================================================ReservaSUM=====================================================--
/**
	Este sp da de alta una reserva de sum, con par·metros indicados seg˙n el usuario. Las validaciones importantes ya son hechas por checks en la tabla dominio.reserva_sum
	@param	fecha_reserva	fecha para la cual se reserva el sum, no de cuando se efect˙a la transacciÛn de la reserva como tal
...
/**
	Este sp borra de manera lÛgica la reserva de sum
...
/**
	...
	@param	hora_hasta		hora donde finaliza la reserva
	@param	valor_hora		valor hora de la reserva de sum
	@param	id_socio		por si se desea cambiar el socio a cargo de la reserva por alg˙n motivo
*/
...
--=====================================================Entrada pileta=====================================================--
/**
	Este sp da de alta una entrada de pileta, las validaciones de campos est·n hechas con checks en la tabla entrada_pileta a la que se hace referencia
	Ac· solo validamos que el monto para invitado y socio sean indicados uno sÌ y el otro no, siempre un monto debe ser indicado, unicamente uno de ellos.
	...
	@param	tipo_entrada_invitado	indica de manera m·s clara, textual, de si la entrada es emitida para un socio o un invitado.
...
/**
	...
	Valida que la factura exista y no estÈ anulada
	Valida que el monto del reembolso no sobrepase al monto de la factura.
	Valida que la fecha del reembolso no sea anterior a la fecha de emisiÛn de la factura, tampoco que sea una fecha a futuro (mayor a la de HOY)
	@motivo_reembolso		motivo del reembolso
	@monto					monto reembolsado
	@id_factura				factura a la cu·l se asociar· el reembolso
...
BEGIN
	...
	BEGIN
		RAISERROR('El motivo del reembolso no puede ser nulo ni vacÌo.', 16, 1);
		RETURN -1;
	END
	...
	BEGIN
		RAISERROR('Error al generar el reembolso, la fecha del reembolso debe estar entre la fecha de emisiÛn de la factura y hoy.', 16, 1);
		RETURN -1;
	END
