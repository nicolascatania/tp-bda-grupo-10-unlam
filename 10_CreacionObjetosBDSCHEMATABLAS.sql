...
BEGIN
	CREATE TABLE dominio.descuento(
		...
		porcentaje DECIMAL (3,2), -- 0,06 0,90 0,50 y asÃ­ tomaran los valores
...
/**
	...
	Valida que la contraseÃ±a cumpla los requerimientos marcados en el check de la tabla usuario, para generar un raiserror con una indicaciÃ³n clara del motivo de falla
	Valida que nombre de usuario sea Ãºnico
	Setea la fecha de creaciÃ³n de contraseÃ±a a hoy y la fecha de expiraciÃ³n a dentro de un aÃ±o
	Encripta la contraseÃ±a
	@param	nombre_usuario indica el nombre de usuario a dar de alta
	@param	contraseÃ±a	   indica la contraseÃ±a a ingresar		
*/
...
BEGIN
    ...
    IF LEN(@contraseña) < 8 OR 
       ...
       (@contraseña NOT LIKE '%!%' AND
        ...
        @contraseña NOT LIKE '%)%')
    BEGIN
        RAISERROR('La contraseÃ±a no cumple con los requisitos de seguridad', 16, 1)
        RETURN
    END
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR('El usuario con ID %d ya estÃ¡ inactivo', 16, 1, @ID_usuario);
            RETURN -1;
        END
...
/**
    El siguiente SP modifica los datos de un usuario existente, puede ser el nombre de usuario o la contraseÃ±a
    @param ID_usuario indica el ID del usuario a modificar (obligatorio para encontrar el usuario en cuestiÃ³n, o terminar si no existe)
    ...
    @param nueva_contraseÃ±a nueva contraseÃ±a (opcional, debe cumplir requisitos) si se indica, se actualizan las fechas de modificado y vencimiento, ademÃ¡s se realizan las validaciones correspondientes
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                RAISERROR('La contraseÃ±a debe tener al menos 8 caracteres, incluir nÃºmeros, letras y un caracter especial (!@#$%^&*)', 16, 1);
                RETURN -1;
            END
...
/*
	...
	@param ID_rol			id para buscar en la tabla, si no existe, cancela la operaciÃ³n
	@param nuevo_nombre_rol	indica el nuevo nombre a setear
	@return 0 Ã©xito, -1 error
*/
