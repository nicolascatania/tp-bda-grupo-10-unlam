...
BEGIN
    ...
    IF LEN(RTRIM(@nombre_membresia)) = 0
        BEGIN 
			RAISERROR('Agregar nombre de membresia.', 16, 1); 
			RETURN; 
		END
    ...
    -- Insercion
    INSERT INTO solNorte.cuota_membresia (mes, anio, monto, nombre_membresia, edad_minima, edad_maxima, id_socio) 
		VALUES (@mes, @anio, @monto, @nombre_membresia, @edad_minima, @edad_maxima, @id_socio);
END;
GO
...
--=====================================================FACTURA=====================================================--
ALTER TABLE solNorte.factura
ALTER COLUMN CUIT_emisor BIGINT;
GO
...
BEGIN 
    ...
    IF NOT EXISTS (SELECT 1 FROM solNorte.factura WHERE ID_factura = @id_factura AND anulada = 0)
...
CREATE OR ALTER PROCEDURE solNorte.eliminar_detalle_factura -- eliminado logico
    @ID_detalle_factura INT
AS
BEGIN
    IF NOT EXISTS (SELECT 1 FROM solNorte.detalle_factura WHERE ID_detalle_factura = @ID_detalle_factura AND borrado = 0)
